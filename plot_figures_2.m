...
plasmids_to_calc_RSS = ["pMM0847","pMM1079"];
...
%% Calculate promoter response to measured pulses of nuclear Msn2(WT|4E|A) w/ K scaling
close all

fraction_active = 1;
% K_scale_list = [0.5:0.1:2,3,4,6,8,10,12];
        ...
        for K_scale_idx = 1:numel(K_scale_list)
            ...
            for n_scale_idx = 1:numel(n_scale_list)
                ...
                parfor condition = 1:numel(condition_list)
                    ...
                    [~,y] = ode45(@(t,y) promoter_ODEs_scale_n(t,y,t_measured,Msn2_measured_temp,promoter_params_temp,K_scale,n_scale,fraction_active),...
...
%% %%%%%%%%%%%%%%%%%%% Save data to avoid recalculating %%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Figure 5A: plot simulated vs measured mCitrine for amplitude conditions
...
clc; clear g; figure('position',[100 100 800 250]);
...
g.set_names('x','','y','','column','','row','','color','','linestyle','');
...
g.facet_wrap(promoter_response_measured_K_scale.reporter,'ncols',4,'scale','independent');
...
% export_fig(fullfile(output_folder,'mCitrine_max_vs_mScarlet_AUC_K_scale_amplitude_conditions'),'-png','-m4');
export_fig(fullfile(pwd,'mCitrine_mutants_measured_vs_scaled_model'),'-png','-m4');

%% Figure S11A: plot RSS/RSS_{min} vs alpha and beta
...
%% Figure S11B: plot simulated vs measured mCitrine for amplitude conditions
%Label selected parameter sets
promoter_response_measured_K_scale.label(:,1) = categorical("NA");
promoter_response_measured_K_scale.label(promoter_response_measured_K_scale.K_scale==1 & ...
    promoter_response_measured_K_scale.n_scale==1)=categorical("Msn2(A)*: w = 1, m = 1");
