...
promoter_response_measured = outerjoin(promoter_response_measured,condition_reorder, 'Type', 'Left', 'MergeKeys', true);
...
model_folder = fullfile(parent_folder,'light_sweep_experiments');
...
%% Calculate promoter response to measured pulses of nuclear Msn2

K_scale_list = [0.5, 1, 2];
promoter_response_measured = cell(numel(reporters_to_plot)*numel(plasmid_list)*n_guesses_plot*numel(K_scale_list)*14,20);
idx = 1;
        ...
        for guess = 1:n_guesses_plot
            ...
            for K_scale_idx = 1:numel(K_scale_list)
                ...
                for condition = 1:14
                    ...
                    promoter_response_measured{idx,7} = t_measured;
                    promoter_response_measured{idx,8} = Msn2_measured_all(:,condition);
                    promoter_response_measured{idx,9} = trapz(t_measured,Msn2_measured_all(:,condition));
                    
                    promoter_response_measured{idx,10} = guess;
                    promoter_response_measured{idx,11} = promoter_params_temp;
                    promoter_response_measured{idx,12} = K_scale;
                    promoter_response_measured{idx,13} = fraction_active;
                    
                    promoter_response_measured{idx,14} = P_unbound_model_temp(:,condition);
                    promoter_response_measured{idx,15} = P_bound_model_temp(:,condition);
                    promoter_response_measured{idx,16} = P_active_model_temp(:,condition);
                    promoter_response_measured{idx,17} = mRNA_model_temp(:,condition);
                    promoter_response_measured{idx,18} = mCitrine_model_temp(:,condition);
                    promoter_response_measured{idx,19} = max(P_bound_model_temp(:,condition));
                    promoter_response_measured{idx,20} = max(mCitrine_model_temp(:,condition));
...
%% Calculate AUC of ideal Msn2 trace
...
%% Calculate amplitude/duration thresholds, slope ratio, and interpulse decay (measurements)
% Initialize
...
%         conditions_to_fit = [1,11,12];
        ...
        % Collect measurements and fit results
...
%% Calculate amplitude and duration threshold vs WT (measurements)
...
%% Calculate amplitude and duration thresholds (vs self, ideal pulses)
...
% Calculate amplitude and duration thresholds vs self
idx = 1;
for reporter_idx = 1:numel(reporter_list)
...
%% Calculate amplitude and duration thresholds vs WT (ideal pulses)
...
%% Calculate slope ratio and interpulse decay (ideal pulses)
...
promoter_response_ideal.P_active_max_k3_forced = promoter_response_ideal.P_active_max_k3;
subset = ismember(promoter_response_ideal.reporter,{'RTN2','TKL2'}) & ...
    ismember(promoter_response_ideal.Msn2,{'Msn2(WT|4E|A)'}) & ...
    promoter_response_ideal.t1<=10;
promoter_response_ideal.P_active_max_k3_forced(subset) = 0;


Msn2_fixed = 1;
promoter_fits.Kd = promoter_fits.K.^promoter_fits.n;
promoter_fits.activation_rate = (promoter_fits.k1.*Msn2_fixed.^promoter_fits.n)./(promoter_fits.Kd + Msn2_fixed.^promoter_fits.n);
promoter_fits.activation_ratio = promoter_fits.activation_rate./promoter_fits.d1;
promoter_fits.on_ratio = promoter_fits.k2./promoter_fits.d2;

promoter_fits_Kd = grpstats(promoter_fits(promoter_fits.rank<=100 & promoter_fits.Msn2=='Msn2(WT|4E|WT)',:),{'reporter'},@(x) mean(log10(x)),'DataVars','Kd');
promoter_fits_Kd = clean_grpstats(promoter_fits_Kd);
promoter_fits_Kd.Properties.VariableNames(end) = {'log10_Kd'};
...
oscillatory_thresholds = promoter_thresholds_measured(promoter_thresholds_measured.Msn2=='Msn2(WT|4E|WT)',{'reporter','slope_ratio','interpulse_decay'});
...
promoter_thresholds_WT = outerjoin(oscillatory_thresholds,single_pulse_thresholds_WT,'Type', 'Left', 'MergeKeys', true);
promoter_thresholds_WT = outerjoin(promoter_thresholds_WT,promoter_fits_Kd,'Type', 'Left', 'MergeKeys', true);
...
% high_senstivity_reporters = promoter_thresholds_WT.reporter(promoter_thresholds_WT.group==1);
...
%% Figure 1B: plot HXK1 induction for Msn2 ± CLASP
close all;
Msn2_to_plot = 'Msn2';
...
g = gramm('x',data_stats.time,'y',100*data_stats.mScarlet_localization,...
    'linestyle',data_stats.CLASP=='CLASP',...
    'subset',data_stats.reporter==reporter_to_plot & data_stats.Msn2==Msn2_to_plot & ismember(data_stats.condition,[1,4]));
...
g.no_legend();
g.draw();
export_fig(fullfile(pwd,'HXK1_induction_Msn2'),'-png','-m4');

clc; clear g; figure('units','normalized','outerposition',[0 0 0.125 0.5]);
g = gramm('x',data_stats.time,'y',data_stats.mCitrine_cell,...
    'linestyle',data_stats.CLASP=='CLASP',...
    'subset',data_stats.reporter==reporter_to_plot & data_stats.Msn2=='Msn2' & ismember(data_stats.condition,[1,4]));
g.facet_grid(data_stats.condition==1,[],'row_labels',false);
g.stat_summary('type','std','setylim',true);
% g.set_color_options('map',[55 55 55]/255);
...
g.no_legend();
g.draw();
export_fig(fullfile(pwd,'HXK1_induction_mCitrine'),'-png','-m4');
...
g = gramm('x',data_light_dose.amplitudes,'y',(data_light_dose.mScarlet_nuclear),...
    ...
    'subset',ismember(data_light_dose.bin,5:6) & data_light_dose.condition<=6);
g.stat_summary('type','std','geom',{'point','errorbar'},'setylim',true,'dodge',0.5,'width',2);
% g.set_names('x','light dose (AU)','y','Nuclear Msn2','column','','row','','linestyle','','color','','marker','');
...
g = gramm('x',data_light_dose.PWM,'y',(data_light_dose.mScarlet_nuclear),...
    'color',data_light_dose.Msn2_tex,'marker',data_light_dose.CLASP,...
    'subset',ismember(data_light_dose.bin,5:6) & data_light_dose.amplitudes==128);
g.stat_summary('type','std','geom',{'point','errorbar'},'setylim',true,'dodge',0.5,'width',2);
% g.set_names('x','','y','Nuclear Msn2','column','','row','','linestyle','','color','','marker','');
g.set_names('x','','y','','column','','row','','linestyle','','color','','marker','');
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
...
%% Figure 2A: Msn2(WT|4E|WT) (aka Msn2*) heatmap
...
for mm = 1:numel(Msn2_list)
    ...
    export_fig(fullfile(output_folder,strcat(string(mm),'_light_sweep_heatmap_WT')),'-png','-m4');
end

%% Figure 2C: amplitude vs duration thresholds (ideal pulses, WT only)
close all
...
g.geom_point();
% g.geom_line();
% g.geom_label('FontSize',10,'FontWeight', 'Bold');
% g.set_names('x','activation timescale (min)','y','amplitude threshold (%)','column','','color','');
...
% g.axe_property('XLim',[-4.5 50],'YLim',[30 100]);
g.no_legend();
g.draw();
...
params_to_plot = {'activation_rate','activation_ratio','on_ratio'};
% reporters_to_plot = {'glpT','RTN2','TKL2','SIP18','ALD3','CTT1','SIP18 D6','DCS2','SIP18 A4','DDR2','HXK1','HSP12'};
...
% g.geom_polygon('x',{[4.5 7.5]},'color',[190 190 190]/255);
g.stat_summary('type','ci','geom',{'bar','black_errorbar'},'setylim',true,'width',0.7,'dodge',0.75);
% g.set_names('x','','y',['log_{10}(K^{n})' newline '(mean ± ci)'],'color','','column','');
g.set_names('x','','y','','color','','column','');
g.axe_property('XTickLabelRotation',45);
...
%% Figure 2E: composite Msn2 vs time (measured vs ideal)

close all
Msn2_ideal = outerjoin(Msn2_ideal,condition_reorder, 'Type', 'Left', 'MergeKeys', true);

% Plot measured vs ideal Msn2 timecourses
clc; clear g; figure('units','normalized','outerposition',[0 0 0.5 0.4]);
...
g.geom_line();
% g.set_names('x','time (min.)','y',['Nuc/cyt Msn2' newline '(mean ± std)'],'column','');
g.set_names('x','time (min.)','y','','column','');
g.set_color_options('map',[125 125 125]/255);
g.set_text_options('Interpreter','tex','base_size',12);
...
g.draw();
g.redraw(0.05);
% export_fig(fullfile(output_folder,'Msn2_localization_measured_vs_ideal'),'-png','-m4');
export_fig(fullfile(pwd,'Msn2_localization_measured_vs_ideal'),'-png','-m4');


close all
numel(unique(data_stats.experiment(data_stats.plasmid=='pMM1079'))) % # experiments

%% Figure 2F: Msn2 AUC per condition (measured vs ideal)
...
g.geom_label();
g.draw();
% export_fig(fullfile(output_folder,'Msn2_AUC_measured_vs_ideal'),'-png','-m4');
export_fig(fullfile(pwd,'Msn2_AUC_measured_vs_ideal'),'-png','-m4');

%% Figure 3A: K scaling timecourse (ideal pulses, WT only)
close all
reporters_to_plot = {'glpT','TKL2','ALD3','SIP18 D6','SIP18 A4','HXK1','RTN2','SIP18','CTT1','DCS2','DDR2','HSP12'};
reporters_to_plot = {'RTN2','DCS2','HSP12'};
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};

% clc; clear g; figure('position',[100 100 1600 500]);
clc; clear g; figure('position',[100 100 700 225]);
g = gramm('x',promoter_response_ideal.time,'y',promoter_response_ideal.mCitrine,...
    'color',promoter_response_ideal.K_scale,...
    'subset',ismember(promoter_response_ideal.reporter,reporters_to_plot) & ...
    ismember(promoter_response_ideal.Msn2,'Msn2(WT|4E|WT)') & ...
    promoter_response_ideal.pulse_label=='duration' & promoter_response_ideal.t1==50);
g.facet_wrap(promoter_response_ideal.reporter,'ncols',6,'scale','independent');
g.stat_summary('type','ci','setylim',true);
% g.set_names('x','time (min)','y',['mCitrine' newline '(mean ± ci)'],'column','','color','');
g.set_names('x','','y','','column','','color','');
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('column',reporters_to_plot,'color',[0.5 1 2]);
g.set_text_options('base_size',11,'interpreter','tex');
% g.geom_polygon('x',{[0 50]},'color',[30 170 220]/255,'alpha',0.1)
g.no_legend();
g.draw();
...
promoter_response_ideal_mean = promoter_response_ideal;
...
%% Figure 3B: A vs WT vs T expression vs time condition 9 (measurements)
close all
% reporters_to_plot = {'glpT','RTN2','TKL2','SIP18','ALD3','CTT1','SIP18 D6','DCS2','SIP18 A4','DDR2','HXK1','HSP12'};
reporters_to_plot = {'RTN2','DCS2','HSP12'};
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};
...
g.stat_summary('type','std','setylim',true);
% g.set_names('x','time (min)','y',['mCitrine' newline '(mean ± std)'],'column','','color','');
g.set_names('x','','y','','column','','color','');
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
...
%% Figure 3C: heatmaps for Msn2(WT|4E|A), Msn2(WT|4E|WT), and Msn2(WT|4E|T)
...
for mm = 1:numel(Msn2_list)
    ...
    set(gcf,'color','w');
    export_fig(fullfile(output_folder,strcat(string(mm),'_light_sweep_heatmap')),'-png','-m4');
end

%% Figure 4A: plot mCitrine vs Msn2(WT|4E|WT) AUC (measured, all mutants)
close all
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};
reporters_to_plot = {'RTN2','HSP12'};
...
% g.facet_wrap(cellstr(mCitrine_stats.reporter),'ncols',6,'column_labels',true,'scale','independent');
...
% Plot interpulse decay
interpulse_conditions = [6,11,13,14];
clc; clear g; figure('position',[100 100 250 400]);
g = gramm('x',mCitrine_stats.mScarlet_AUC,'y',mCitrine_stats.mCitrine_max,...
    'color',mCitrine_stats.Msn2_tex,...
    'subset',ismember(mCitrine_stats.condition,interpulse_conditions) & ...
    ismember(mCitrine_stats.Msn2,Msn2_to_plot) & ismember(mCitrine_stats.reporter,reporters_to_plot));
g.facet_grid(mCitrine_stats.reporter,[],'scale','independent','row_labels',false);
g.stat_summary('type','std','geom',{'point','errorbar'},'setylim',true,'width',3,'dodge',1);
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('row',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
g.set_text_options('base_size',10,'interpreter','tex','title_scaling',1);
g.set_names('x','Msn2 AUC','y','max mCitrine','color','','column','','marker','continuous(1) vs pulsed(0)','linestyle','continuous(1) vs pulsed(0)');
g.set_point_options('base_size',6);
% g.stat_fit('fun',@(a,b,x) a*exp(b*x),'StartPoint',[1,0.1],'intopt','functional');
g.stat_fit('fun',@(a,b,x) exp(a*x) + b,'StartPoint',[1,0.1],'intopt','functional','disp_fit',false);
g.axe_property('xdir','reverse','XLim',[17.75 29] );
g.no_legend();
g.draw();
for ii = 1:size(g.results.stat_fit,1)
    g.results.stat_fit(ii).area_handle.FaceAlpha = 0.03;
end
g.facet_axes_handles(end).YLim = [0 1750];

%% Figure 4B: plot slope ratio w/ variance
reporters_to_plot = {'glpT','RTN2','TKL2','SIP18','ALD3','CTT1','SIP18 D6','DCS2','SIP18 A4','DDR2','HXK1','HSP12'};
% Msn2_to_plot = {'Msn2(WT|4E|WT)'};
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};
nonresponders = (promoter_thresholds_measured.Msn2=='Msn2(WT|4E|T)' & ...
...
% g.set_names('x','','y',['slope ratio' newline '(mean ± std)'],'column','','color','');
g.set_names('x','','y','','column','','color','');
g.set_order_options('x',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
% g.set_color_options('map',[125 125 125]/255);
...
for r = 6%:numel(reporter_list)
    ...
    threshold = prctile(data.mCitrine_cell(subset,:),99);
    
    subset = ismember(data.experiment,experiments_to_plot) & ismember(data.Msn2,'Msn2(WT|4E|T)') & ...
        data.condition==6 & data.time>115 & data.time<125;
    T_6 = sum(data.mCitrine_cell(subset,:)>threshold)./size(data(subset,:),1);
    
        subset = ismember(data.experiment,experiments_to_plot) & ismember(data.Msn2,'Msn2(WT|4E|T)') & ...
        data.condition==11 & data.time>115 & data.time<125;
    T_11 = sum(data.mCitrine_cell(subset,:)>threshold)./size(data(subset,:),1);
    
    close all;
%     clc; clear g; figure('units','normalized','outerposition',[0 0 0.5 0.35]);
    clc; clear g; figure('position',[100 100 550 150]);
    g = gramm('x',log10(data.mCitrine_cell),...
        'color',cellstr(data.Msn2_tex),...
        'subset',~exclude & ismember(data.experiment,experiments_to_plot) & ismember(data.Msn2,Msn2_to_plot) & ...
...
%     g.set_names('x','log(mCitrine)','y','%','column','','row','','linestyle','','color','');
    g.set_names('x','','y','','column','','row','','linestyle','','color','');    
    g.set_order_options('color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
    g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
    g.set_text_options('base_size',10,'interpreter','tex','title_scaling',1);
    g.no_legend();
%     g.geom_vline('xintercept',log10(threshold))
    g.draw();
    disp(strcat("T_continous: ", num2str(100*T_6),"% > threshold"))
    disp(strcat("T_pulsed: ", num2str(100*T_11),"% > threshold"))
    export_fig(fullfile(pwd,'HSP12_pulsed_vs_continuous_hist_stairs'),'-png','-m4');

end

%% Figure S1D: plot basal mCitrine histograms
close all
Msn2_to_plot = {'Msn2','Msn2(WT|4E|WT)'};
...
% g.facet_grid(data.CLASP,[],'scale','independent');
...
numel(unique(data_basal.experiment(data_basal.plasmid=='pMM0845'))) % # experiments
numel(unique(data_basal.experiment(data_basal.plasmid=='pMM0847'))) % # experiments

%% Figure S2 - S4 Plot fits over mCitrine timecourses
close all;
...
%% Figure S5A: plot light programs

%Import example light program
clc
led_program = load(fullfile(model_folder,'example_led_program.mat'));
...
%% Figure S5B: plot Msn2 localization (all mutants)
...
g.facet_wrap(data_stats.condition_display,'ncols',7,'column_labels',false);
g.stat_summary('type','std','setylim',true);
g.set_names('x','time (min)','y','Msn2 amplitude (%)','color','');
g.set_text_options('base_size',10,'interpreter','tex','title_scaling',1);
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('row',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
g.no_legend();
...
%% Figure S5D: plot reporter induction for full light CLASP vs dCLASP
close all;
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};
% reporters_to_plot = {'glpT','TKL2','ALD3','SIP18 D6','DCS2','HXK1','RTN2','SIP18','CTT1','SIP18 A4','DDR2','HSP12'};
reporters_to_plot = {'glpT','RTN2','TKL2','SIP18','ALD3','CTT1','SIP18 D6','DCS2','SIP18 A4','DDR2','HXK1','HSP12'};
...
%% Figure S6A Plot transition rate as a function of Msn2 with varying K and n
...
plot(x,Hill(x,1,1,1),'r')
legend('K = 0.5','K = 0.25','K = 1','Location','northwest')
title('k1 = 1, n = 1')
ylim([0 1.1])
xlim([0 1.1])
xlabel('[Msn2]')
...
figure; hold on
plot(x,Hill(x,1,1,0.5),'k')
plot(x,Hill(x,1,0.5,0.5),'g')
plot(x,Hill(x,1,2,0.5),'r')
legend('n = 1','n = 0.5','n = 2','Location','northwest')
title('k1 = 1, K = 0.5')
ylim([0 1.1])
xlim([0 1.1])
xlabel('[Msn2]')
...
%% Figure S6B: Plot top 100 params for all promoters
close all

reporters_to_plot = {'RTN2','TKL2','SIP18','ALD3','CTT1','SIP18 D6','DCS2','SIP18 A4','DDR2','HXK1','HSP12'};
% reporters_to_plot = {'CTT1','SIP18 D6','DCS2'};
promoter_fits.Kd = promoter_fits.K.^promoter_fits.n;
params_to_plot = {'k1','d1','k2','K','n','Kd','d2','k3'};
% params_to_plot = {'K','n','Kd'}
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};
Msn2_to_plot = {'Msn2(WT|4E|WT)'}
...
for ii = 1:numel(params_to_plot)
    ...
    %         g(1,ii).set_order_options('x',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
    %         g(1,ii).set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
...
%% Figure S6C: plot amp_thresh_vs_WT and dur_thresh_vs_WT calculation example
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};
reporter = 'DDR2';
close all
clc; clear g; figure('position',[100 100 400 300]);
...
g.set_point_options('base_size',2);
g.set_text_options('interpreter','tex','base_size',11);
% g.geom_hline('yintercept',0.04);
% g.geom_vline('xintercept',40.58);
% g.geom_vline('xintercept',55.13);
% g.geom_vline('xintercept',93.12);
g.no_legend();
g.draw();
g.redraw(0.05);
...
g.set_point_options('base_size',2);
g.set_text_options('interpreter','tex','base_size',11);
% g.geom_hline('yintercept',0.043);
% g.geom_vline('xintercept',4.73);
% g.geom_vline('xintercept',7.92);
% g.geom_vline('xintercept',45.84);
g.no_legend();
g.draw();
g.redraw(0.05);

%% Figure S7C: plot Msn2 ideal - Msn2 measured
close all

% Plot measured vs ideal Msn2 AUC
clc; clear g; figure('position',[100 100 500 300]);
g = gramm('x',data_Msn2_AUC.time,'y',data_Msn2_AUC.Msn2_localization_exclude,...
    'color',data_Msn2_AUC.condition_display,...
    'subset',data_Msn2_AUC.condition<=9);
g.geom_line();
g.set_names('x','time (min)','y','Msn2_{ideal} -    Msn2_{measured}','color','light program');
g.set_text_options('base_size',10,'interpreter','tex','title_scaling',1);
% g.set_continuous_color()
% g.axe_property('XLim',[-10 60]);
g.geom_vline('xintercept',14)
g.draw();

%% Figure S7D: significance test second pulses of interpulse duration conditions
close all hidden
clc

condition_11_pulse_1 = data_stats.mScarlet_localization(data_stats.condition_display==11 & ismember(data_stats.bin,[5,6]));
condition_13_pulse_1 = data_stats.mScarlet_localization(data_stats.condition_display==13 & ismember(data_stats.bin,[5,6]));
condition_14_pulse_1 = data_stats.mScarlet_localization(data_stats.condition_display==14 & ismember(data_stats.bin,[5,6]));

condition_11_pulse_2 = data_stats.mScarlet_localization(data_stats.condition_display==11 & ismember(data_stats.bin,[9,10]));
condition_13_pulse_2 = data_stats.mScarlet_localization(data_stats.condition_display==13 & ismember(data_stats.bin,[11,12]));
condition_14_pulse_2 = data_stats.mScarlet_localization(data_stats.condition_display==14 & ismember(data_stats.bin,[13,14]));


[h,p] = ttest2(condition_11_pulse_2,condition_13_pulse_2)
...
data_stats_anova = data_stats;
data_stats_anova.pulse(:,1) = nan;
pulse_1_subset = (...
    (ismember(data_stats_anova.bin,[5,6]) & data_stats_anova.condition_display==11) | ...
    (ismember(data_stats_anova.bin,[5,6]) & data_stats_anova.condition_display==13) | ...
    (ismember(data_stats_anova.bin,[5,6]) & data_stats_anova.condition_display==14)...
    );
data_stats_anova.pulse(pulse_1_subset) = 1;
pulse_2_subset = (...
    (ismember(data_stats_anova.bin,[9,10]) & data_stats_anova.condition_display==11) | ...
    (ismember(data_stats_anova.bin,[11,12]) & data_stats_anova.condition_display==13) | ...
    (ismember(data_stats_anova.bin,[13,14]) & data_stats_anova.condition_display==14)...
    );
data_stats_anova.pulse(pulse_2_subset) = 2;
data_stats_anova = data_stats_anova(pulse_1_subset | pulse_2_subset,:);

data_stats_anova.group = strcat("condition ",string(data_stats_anova.condition_display)," pulse ",string(data_stats_anova.pulse));
data_stats_anova = sortrows(data_stats_anova,{'pulse','condition'});
[p, tbl, stats] = anova1(data_stats_anova.mScarlet_localization,data_stats_anova.group);

[a b c d] = multcompare(stats)

clear g; figure
g = gramm('x',categorical(data_stats_anova.pulse),'y',100*data_stats_anova.mScarlet_localization,...
    'color',data_stats_anova.pulse_low_times/60);
g.stat_boxplot('notch',true);
g.set_names('x','pulse','y','Msn2 amplitude (%)','color','');
g.set_text_options('Interpreter','tex');
g.set_point_options('base_size',2)
g.draw()

%% Figure S8B: plot amplitude and duration thresholds vs WT w/ variance (ideal pulses) ****
reporters_to_plot = {'glpT','RTN2','TKL2','SIP18','ALD3','CTT1','SIP18 D6','DCS2','SIP18 A4','DDR2','HXK1','HSP12'};
Msn2_to_plot = {'Msn2(WT|4E|A)','Msn2(WT|4E|WT)','Msn2(WT|4E|T)'};
...
% g.facet_wrap(promoter_thresholds_ideal_mean.Msn2,'ncols',3);
...
%% Figure S8C: measured SIP18 and TKL2 vs predicted (ideal and measured pulses) 
% reporters_to_plot = {'glpT','TKL2','ALD3','SIP18 D6','DCS2','HXK1','RTN2','SIP18','CTT1','SIP18 A4','DDR2','HSP12'};
reporters_to_plot = {'RTN2','TKL2','SIP18','SIP18 A4','DDR2','HSP12'};

close all
clc; clear g; figure('position',[100 100 600 400]);
g = gramm('x',mCitrine_stats.mScarlet_AUC,'y',mCitrine_stats.mCitrine_max,...
    'color',mCitrine_stats.Msn2_tex,...
    'subset',ismember(mCitrine_stats.condition,[1,2:2:8,9]));
g.facet_wrap(mCitrine_stats.reporter,'ncols',3,'scale','independent');
g.stat_summary('type','std','geom',{'point','errorbar'},'setylim',true);
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('column',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
g.set_text_options('Interpreter','tex');
g.set_names('x','duration (min)','y','max mCitrine','color','','column','');
g.no_legend();
g.draw();

g.update('x',promoter_response_measured.mScarlet_AUC,'y',promoter_response_measured.mCitrine_max,...
    'color',promoter_response_measured.Msn2_tex,...
    'subset',ismember(promoter_response_measured.condition,[1,2:2:8,9]) & promoter_response_measured.K_scale==1);
g.facet_wrap(promoter_response_measured.reporter,'ncols',3,'scale','independent');
...
g.update('x',promoter_response_ideal.mScarlet_AUC,'y',promoter_response_ideal.mCitrine_max,...
    'color',promoter_response_ideal.Msn2_tex,...
    'subset',ismember(promoter_response_ideal.pulse_idx,1:13) & promoter_response_ideal.K_scale==1);
g.facet_wrap(promoter_response_ideal.reporter,'ncols',3,'scale','independent');
g.stat_summary('setylim',true);
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('column',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
g.set_text_options('Interpreter','tex');
...
g.stat_summary('type','std','setylim',true);
% g.set_names('x','time (min)','y','mCitrine' ,'column','','color','');
g.set_names('x','','y','' ,'column','','color','');
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('column',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
g.set_text_options('base_size',11,'interpreter','tex');
% g.geom_polygon('x',{[0 5];[10 15];[20 25];[30 35];[40 45];[50 55]},'color',[30 170 220]/255,'alpha',0.1)
g.no_legend();
g.draw();
% g.redraw(0.025)
...
%% Figure S10A: plot mCitrine noise 
...
% g.facet_wrap(mCitrine_stats.condition,'ncols',6);
g.stat_summary('type','std','geom',{'point'},'width',0.7,'dodge',0.75,'setylim',true);
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('x',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
% g.set_names('x','','y','noise','color','','column','','row','');
g.set_names('x','','y','','color','','column','','row','');
g.set_point_options('markers',{'o'},'base_size',6);
% g.axe_property('XTickLabelRotation',45,'YScale','log','YLim',[0.08 10]);
...
%% Figure S10B: max mCitrine vs noise
...
% g.facet_wrap(mCitrine_stats.Msn2,'ncols',6);
g.geom_point();
g.geom_label();
% g.stat_summary('type','std','geom',{'bar','black_errorbar'},'width',0.7,'dodge',0.75);
g.set_color_options('map',plot_colors.RGB(ismember(plot_colors.Msn2,Msn2_to_plot),:)/255);
g.set_order_options('x',reporters_to_plot,'color',plot_colors.Msn2_tex(ismember(plot_colors.Msn2,Msn2_to_plot)));
g.set_names('x','log_{10}(max mCitrine)','y','log_{10}(noise)','color','','column','','row','');
% g.axe_property('XScale','log','YScale','log');
% g.axe_property('XTickLabelRotation',45,'YLim',[0 9]);
g.set_text_options('interpreter','tex','base_size',11);
% g.stat_glm();
g.no_legend();
g.draw()
